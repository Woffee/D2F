...
class _WikiDataSPARQLBase(_WikiDataBase):
    ...
    @staticmethod
    def _request_all_properties_with_time(entity_id):
        query = """
            PREFIX wd: <http://www.wikidata.org/entity/>
            PREFIX wdt: <http://www.wikidata.org/prop/direct/>
            ...
            SELECT DISTINCT ?property ?propertyLabel ?object ?objectLabel ?instance_of ?instanceOfLabel ?time ?start_time ?end_time WHERE {
              wd:<ENTITY> ?property ?statement .
              ?statement ?ps ?object .
              ?object wdt:P31 ?instance_of .
              ...
              # Handle wdt: properties by converting them to p: properties
              BIND(IF(STRSTARTS(STR(?property), "http://www.wikidata.org/prop/direct/"),
                       IRI(REPLACE(STR(?property), "http://www.wikidata.org/prop/direct/", "http://www.wikidata.org/entity/")),
                       ?property) AS ?propertyEntity)
            
              # Fetch the property label
              OPTIONAL { ?propertyEntity rdfs:label ?propertyLabel FILTER (lang(?propertyLabel) = "en") }
            
              # Fetch the labels for object and instance_of
              OPTIONAL { ?object rdfs:label ?objectLabel FILTER (lang(?objectLabel) = "en") }
              OPTIONAL { ?instance_of rdfs:label ?instanceOfLabel FILTER (lang(?instanceOfLabel) = "en") }
            }
...
class Entity(_WikiDataSPARQLBase):
    ...
    def all_properties(self):
        return self._request_all_properties_with_time(self.idx)
